# Problem 32: Custom Dynamic Linker

## Description
Simulate the lazy binding process of shared libraries (PLT/GOT).

## Learning Objectives
- **Concepts**: ELF Dynamic Section, Relocation, Lazy Binding
...
- **Custom Dynamic Linker** appears in real systems as a building block for reliability, performance, or scalability.
...
## Solution Summary
The dynamic linker simulation implements lazy binding by maintaining a Procedure Linkage Table (PLT) and Global Offset Table (GOT). On first call, the PLT stub triggers the linker to resolve the symbol address and update the GOT entry; subsequent calls jump directly through the GOT, mimicking how shared libraries defer symbol resolution until runtime.

**[View Solution Code](https://github.com/yunquleonliu/SystCode/blob/main/problems/32_dynamic_linker/solution.c)**
