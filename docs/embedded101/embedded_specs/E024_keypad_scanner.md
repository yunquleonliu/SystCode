...
- Think of **Keypad Scanner** as a system behavior problem before it is a coding problem.
...
- This spec serves as the detailed draft for E024.
...
uint32_t key = hash_u32(0x018u);
uint32_t idx = key % 4u;
uint32_t selected = nodes[idx];
int ok = selected == nodes[idx];
printf("[E024] dist_core key=%u node=%u %s\n", key, selected, ok ? "PASS" : "FAIL");
return ok ? 0 : 1;
```


## Reference Implementation
- C source: [../../problems/embedded101/E024_keypad_scanner/solution.c](https://raw.githubusercontent.com/yunquleonliu/SystCode/main/problems/embedded101/E024_keypad_scanner/solution.c)
